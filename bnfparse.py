...
class Tree:
    ...
    def __str__(self):
        return re.sub(r'\s+', ' ', ' '.join(self.generate()[1:])).strip()
    
    def generate(self, output=[]):
        """Traverse the tree to generate a sentence licensed by the FSG."""
        if self.attrib.has_key('token'):    # Rule or terminal
            output.append(self.attrib['token'])
            for child in self.children:
                child.generate(output)
        if self.attrib.has_key('<all-of>'): # Conjunction
            for child in self.children:
                child.generate(output)
        if self.attrib.has_key('<one-of>'): # Disjunction
            chosen = random.choice(self.children)
            chosen.generate(output)
        if self.attrib.has_key('repeat'):   # Repetition
            n = random.choice(self.attrib['repeat'])
            for i in range(n):
                for child in self.children:
                    child.generate(output)
        return output
